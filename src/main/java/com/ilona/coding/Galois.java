...
public final class Galois {
    ...
    public static final byte[] EXP_TABLE = new byte[]{
    ...
    /**
     * Calculates max primitive.
     */
    public static byte[] getMaxPrimitive() {
        Map<Integer, byte[]> primitives = getPrimitives();
        byte[] primitive = null;
        for (int i = 0; i < 256; i++) {
            if (primitives.get(i) != null) primitive = primitives.get(i);
        }
        return primitive;
    }

    /**
     * Calculates primitives.
     */
    public static Map<Integer, byte[]> getPrimitives() {
        Map<Integer, byte[]> primitives = new HashMap<>();
        Set<Byte> set;
        for (int i = 0; i < 256; i++) {
            byte[] primitive = new byte[256];
            set = new HashSet<>();
            for (int j = 0; j < FIELD_SIZE; j++) {
                primitive[j] = exp((byte) i, j);
                set.add(primitive[j]);
            }
            if (set.size() == FIELD_SIZE - 1)
                primitives.put(i, primitive);
        }
        return primitives;
    }
